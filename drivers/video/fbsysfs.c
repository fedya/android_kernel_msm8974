...
static ssize_t store_blank_force(struct device *device,
			   struct device_attribute *attr,
			   const char *buf, size_t count)
{
	struct fb_info *fb_info = dev_get_drvdata(device);
	...
	blank = simple_strtoul(buf, &last, 0);
	...
	if (blank) {
		err = fb_blank(fb_info, blank);
		fb_info->force_blanked = true;
	}
	else {
		fb_info->force_blanked = false;
		err = 0;
	}
...
static ssize_t show_blank_force(struct device *device,
			  struct device_attribute *attr, char *buf)
{
    struct fb_info *fb_info = dev_get_drvdata(device);
    return snprintf(buf, PAGE_SIZE, "%d\n", fb_info->force_blanked);
}
...
static struct device_attribute device_attrs[] = {
	...
	__ATTR(blank_force, S_IRUGO|S_IWUSR, show_blank_force, store_blank_force),
