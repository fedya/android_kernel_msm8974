...
int systemRun(int call)
{
	u16 reg0A, reg230, cntr = 0;
	s1d13522_ioctl_cmd_params cmd_params;
	dbg_info("vacomVal in %s() is %d\n", __func__, vcomVal);
	if (call == NON_IOCTL_CALL) {
		if ((sleep_state == EPD_RUNNING) || (sleep_state == EPD_RUNNING_IOCTL))
			return 0;
		else
			sleep_state = EPD_RUNNING;
	}
...
int systemSleep(int call)
{
	unsigned int reg0A;
	s1d13522_ioctl_cmd_params cmd_params;
	if (call == NON_IOCTL_CALL) {
		if (sleep_state == EPD_RUNNING_IOCTL)
			return 0;
		else if (sleep_state == EPD_SLEEP)
			return -1;
	}
...
static int s1d13522fb_EPDInit(void)
{
...
static int s1d13522fb_systemSleep(struct s1d13522fb_par *par, unsigned int cmd)
{
	dbg_info("[%d]EPD Sleep Started..!!!\n", __LINE__);
	mutex_lock(&(par->io_lock));
	dbg_info("[%d]EPD Sleep Started..Lock Acquired!!!\n", __LINE__);
	if (sleep_state == EPD_SLEEP) {
		dbg_info("EPD System already in Sleep State(lazy call)!!!\n");
		mutex_unlock(&(par->io_lock));
		return 0;
	}
	systemSleep(IOCTL_CALL);
	mutex_unlock(&(par->io_lock));
	dbg_info("[%d]EPD Sleep Successful..Lock Freed!!!\n", __LINE__);
	return 0;
}

static void s1d13522fb_systemRun(struct s1d13522fb_par *par, unsigned int cmd)
{
	dbg_info("[%d]EPD Wake-up Started..!!!\n", __LINE__);
	mutex_lock(&(par->io_lock));
	dbg_info("[%d]EPD Wake-up Started..Got the lock!!!\n", __LINE__);
	sleep_state = EPD_RUNNING_IOCTL;
	systemRun(IOCTL_CALL);
...
static void s1d13522fb_SetTemp(signed short temperature)
{
	...
	dbg_info("[%d:%s] val of temperature to write %d state is %d\n",
				__LINE__, __func__ , temperature, sleep_state);
	mutex_lock(&(par->io_lock));
        if (-1 == systemRun(NON_IOCTL_CALL))
                return ;
...
{
	dbg_info("[%s:%d] Calling s1d13522fb_display_init\n", __func__, __LINE__);
	s1d13522fb_display_init(UPD_FULL, WF_MODE_INIT, par);

        INIT_DELAYED_WORK(&(dw), s1d13522fb_display_init_wq);
					...
					int offset, int numpages)
{
	...
	dbg_info("Sleep State (0: RUN STATE 1: SLEEP STATE):[%d] Offset:%d\n",
						sleep_state, offset);
	...
	/*printk(KERN_ERR "%s() Mode:%d WF:%d\n",
			__func__, updateMode, updateWaveform);*/
#if 0
...
void s1d13522fb_dpy_update(struct s1d13522fb_par *par)
{
	...
	dbg_info("Sleep State(0: RUN STATE 1: SLEEP STATE):[%d]\n", sleep_state);
			    ...
			    unsigned long arg)
{
	...
	dbg_info("%s:cmd=%04Xh SleepState[0~Active 1~Sleep]:%d HRDYState:%d\n",
					__func__, cmd, sleep_state, state);
	...
	if ((cmd == S1D13522_RUN_SYS && sleep_state == EPD_SLEEP) ||
			(cmd == S1D13522_SLP && sleep_state > EPD_SLEEP))
		goto L1;
	else if ((sleep_state == EPD_SLEEP) && ((cmd == S1D13522_GET_TEMP) ||
			(cmd == S1D13522_SET_TEMP) ||
			(cmd == S1D13522_SLP))) {
		dbg_info("%s()[%d]: Command[%04Xh] Not allowed\n",
	...
	default:
		if (epdCheck == -1 || sleep_state == EPD_SLEEP)
			return -EINVAL;
	}
...
L1:
	...
	case S1D13522_GET_PHONE_COLOR:
		return device_color;

		/* zero parameter commands: */
	case S1D13522_RUN_SYS:
		s1d13522fb_systemRun(par, cmd);
		break;
	case S1D13522_STBY:
	case S1D13522_SLP:
		s1d13522fb_systemSleep(par, cmd);
		break;
