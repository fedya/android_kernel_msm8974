	...
	struct reg_settings_t *settings)
{
	...
	for (i = 0; i < size; i++) {
                 ...
                 {
                   msleep(settings[i].reg_data);
		   CDBG("after %d register setting,waiting %d ms\n",i,settings[i].reg_data);
                   continue;
                 }
#endif
		switch (type) {
...
static int32_t msm_actuator_init(struct msm_actuator_ctrl_t *a_ctrl,
	...
	uint16_t i = 0;
#ifdef CONFIG_AK7345
	enum msm_actuator_data_type init_data_type=MSM_ACTUATOR_BYTE_DATA;
#endif
	...
	if (set_info->actuator_params.init_setting_size) {
		if (
#ifdef CONFIG_AK7345
			a_ctrl->func_tbl->actuator_init_focus&&!init_actuator
#else
			a_ctrl->func_tbl->actuator_init_focus
#endif
		   ) {
